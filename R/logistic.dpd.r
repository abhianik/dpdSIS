...
logistic.dpd <- function(y,X,alpha,method = "L-BFGS-B",p=ncol(X),Initial=matrix(rep(1,p))) {

  n <- length(y)
  T<-solve(t(X)%*%X)

#Objective function for the computation of MDPDE (Minimum DPD estimators)
objfunction_logistic_reg <- function(t) {
    ...
    for (i in 1:n) {
      eta<-exp(X[i,]%*%t)
      pi<-eta/(1+eta)
      fa[i,] <- log(1+eta) - (y[i] *(X[i,] %*%t))
      fb[i,] <- dbinom(y[i],1,pi, log = FALSE)^alpha
      fc[i,]=(1+ eta^(1+alpha))/((1+eta)^(1+alpha))
    }
    if ( alpha == 0) 
    { f = sum(fa)}
    else
...
## Default method used in optim is "L-BFGS-B", in case of non-convergence, this method can be changed
  result<-optim(Initial, objfunction_logistic_reg , gr = NULL,
              method =  method,
